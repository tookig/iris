...
const groupWindowNormal = 'bg-slate-600'
const groupWindowEdit = 'bg-blue-600'
...
function create (_args) {
  ...
  editButton.on('click', e => {
    e.preventDefault()
    if (isEditing) {
      newWindow.header.removeClass(groupWindowEdit).addClass(groupWindowNormal)
      ...
      editButton.find('span').text('Börja ändra')
    } else {
      newWindow.header.addClass(groupWindowEdit).removeClass(groupWindowNormal)
      ...
      editButton.find('span').text('Sluta ändra')
    }
    isEditing = !isEditing
  })
